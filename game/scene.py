    ...
    def set_scene(self):
        ...
        for i in range(count):
            segment_shape_bottom = pymunk.Segment(
                ...
                self.map.border["coordinates"][i],
                self.map.border["coordinates"][(i + 1) % count],
                2,
            )
        ...
        for circle in self.map.circles:
            ...
            circle_shape = pymunk.Circle(
                circle_body, self.map.circles[circle]["radius"]
            )
        ...
        for rectangle in self.map.rectangles:
            ...
            box_shape = pymunk.Poly(
                box_body, self.map.rectangles[rectangle]["coordinates"]
            )
